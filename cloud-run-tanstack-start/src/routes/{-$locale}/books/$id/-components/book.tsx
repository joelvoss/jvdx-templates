...
const Route = getRouteApi("/{-$locale}/books/$id/");
 ...
 */
function BookHeader(props: { book: BookRecord }) {
	const t = useTranslations("routes.books.details");

	return (
		<div className="mb-6 flex items-start justify-between">
			<h1 className="text-3xl font-bold text-gray-900">{props.book.title}</h1>
			<div className="flex gap-2">
				<Link
					to="/{-$locale}/books/$id/edit"
...
function BookMetadata(props: { book: BookRecord }) {
	...
	return (
		<div className="mt-8 border-t border-gray-200 pt-6 text-sm text-gray-500">
			<p>{t("created", { date: formatDate(props.book.createdAt) })}</p>
			<p>{t("updated", { date: formatDate(props.book.updatedAt) })}</p>
		</div>
	);
}
