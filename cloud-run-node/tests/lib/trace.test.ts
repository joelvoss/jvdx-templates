...
describe('trace', () => {
	...
	let parseLegacyTraceContext: typeof import('~/lib/trace').parseLegacyTraceContext;
	...
	beforeEach(async () => {
		...
		parseLegacyTraceContext = traceModule.parseLegacyTraceContext;
	});
	...
	test('uses X-Cloud-Trace-Context when traceparent is unavailable', async () => {
		let app = new Hono<{ Variables: Variables }>();
		app.use(traceMiddleware({ projectId: 'test-project-id' }));
		...
		app.get('/', (c) =>
			c.json({
				id: c.get('traceId'),
				sampled: Boolean(
					(trace.getActiveSpan()?.spanContext().traceFlags ?? 0) & 1,
				),
			}),
		);
		...
		let res = await app.request('/', {
			headers: {
				'x-cloud-trace-context': '1234567890abcdef1234567890abcdef/42;o=1',
			},
		});
		let body = (await res.json()) as { id: string; sampled: boolean };
	...
	test('preserves unsampled legacy trace context', async () => {
		let app = new Hono<{ Variables: Variables }>();
		app.use(traceMiddleware({ projectId: 'test-project-id' }));
		...
		let res = await app.request('/', {
			headers: {
				'x-cloud-trace-context': '1234567890abcdef1234567890abcdef/42;o=0',
			},
		});
		let body = (await res.json()) as { id: string; sampled: boolean };
	...
	test('prefers traceparent over X-Cloud-Trace-Context', async () => {
		let app = new Hono<{ Variables: Variables }>();
		app.use(traceMiddleware({ projectId: 'test-project-id' }));

		app.get('/', (c) => c.json({ id: c.get('traceId') }));

		let res = await app.request('/', {
			headers: {
				traceparent: '00-fedcba0987654321fedcba0987654321-1234567890abcdef-00',
				'x-cloud-trace-context': '1234567890abcdef1234567890abcdef/42;o=1',
			},
		});
		let body = (await res.json()) as { id: string };
		...
		expect(body).toStrictEqual({
			id: 'projects/test-project-id/traces/fedcba0987654321fedcba0987654321',
		});
	});

	test('preserves an unsampled traceparent over a sampled legacy header', async () => {
		let app = new Hono<{ Variables: Variables }>();
		app.use(traceMiddleware({ projectId: 'test-project-id' }));
	...
	test('falls back to legacy context when traceparent is malformed', async () => {
		let app = new Hono<{ Variables: Variables }>();
		app.use(traceMiddleware({ projectId: 'test-project-id' }));
	...
	test('parses legacy trace and span IDs using OpenTelemetry formats', () => {
		expect(
			parseLegacyTraceContext('ABCDEFABCDEFABCDEFABCDEFABCDEFAB/1;o=1'),
		).toEqual({
		...
		expect(
			parseLegacyTraceContext(
				'1234567890abcdef1234567890abcdef/18446744073709551615',
			),
		).toEqual({
	...
	test.each([
		'',
		'1234567890abcdef1234567890abcde/1;o=1',
		'00000000000000000000000000000000/1;o=1',
		'1234567890abcdef1234567890abcdef/0;o=1',
		'1234567890abcdef1234567890abcdef/18446744073709551616;o=1',
		'1234567890abcdef1234567890abcdef/not-a-number;o=1',
		'1234567890abcdef1234567890abcdef;o=1',
	])('ignores invalid legacy trace context: %s', (header) => {
		expect(parseLegacyTraceContext(header)).toBeUndefined();
	});
...
describe('shutdownTracing', () => {
	...
	test('does not export spans when tracing is disabled', async () => {
		vi.stubEnv('OTEL_TRACES_EXPORTER', 'none');
		let consoleSpy = vi.spyOn(console, 'dir').mockImplementation(() => {});

		let { shutdownTracing, tracer } = await import('../../src/lib/trace');

		let span = tracer.startSpan('disabled.span');
		span.end();
		await shutdownTracing();
